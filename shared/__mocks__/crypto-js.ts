...
class WordArray {
  ...
  toString(encoder?: typeof Hex | typeof Base64): string {
    if (encoder === Hex) {
...
class CipherParams {
  ...
  toString(formatter?: { stringify: (cipherParams: CipherParams) => string }): string {
...
function PBKDF2(password: string, salt: WordArray | string, cfg: { keySize?: number; iterations?: number }): WordArray {
...
// AES implementation
const AES = {
  encrypt: (message: WordArray | string, key: WordArray, cfg?: { iv?: WordArray }): { ciphertext: WordArray; iv: WordArray; toString: () => string } => {
    try {
  ...
  decrypt: (cipherParams: { ciphertext: WordArray; iv?: WordArray }, key: WordArray, cfg?: { iv?: WordArray }): WordArray => {
    try {
      // Convert key WordArray to bytes
      const keyBytes = new Uint8Array(32); // 256-bit key
      for (let i = 0; i < Math.min(32, key.sigBytes); i++) {
