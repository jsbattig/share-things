...
# Run servers in the background and then run tests
...
echo "Checking if port 3000 (API server) is available..."
...
echo "Checking if port 3001 (client) is available..."
while ! nc -z localhost 3001 >/dev/null 2>&1; do
  echo "Waiting for client to start..."
  sleep 1
done
echo "Client is running on port 3001"
