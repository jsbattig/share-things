...
# Run session tests in the background and then run tests without waiting for user input
...
echo "Running session E2E browser tests..."
npx playwright test test/e2e/browser/tests/session.test.ts --config=test/e2e/browser/playwright.config.ts --project=chromium --reporter=list
