...
$DOCKER_COMPOSE_CMD -f ../config/podman-compose.yml -f ../config/podman-compose.prod.yml down
...
cat > ../config/podman-compose.prod.temp.yml << EOL
...
$DOCKER_COMPOSE_CMD -f ../config/podman-compose.prod.temp.yml build
BUILD_EXIT_CODE=$?
...
if [ $BUILD_EXIT_CODE -ne 0 ]; then
    if [ -n "$CI" ]; then
        ...
        rm ../config/podman-compose.prod.temp.yml
        exit 1
    fi
fi
...
if [ $BUILD_EXIT_CODE -ne 0 ] && [ -n "$CI" ]; then
    ...
    $DOCKER_COMPOSE_CMD -f ../config/podman-compose.prod.temp.yml up -d
    START_EXIT_CODE=$?

    if [ $START_EXIT_CODE -ne 0 ]; then
        echo -e "${RED}Failed to start production containers. Exiting.${NC}"
        $DOCKER_COMPOSE_CMD -f ../config/podman-compose.prod.temp.yml down
        rm ../config/podman-compose.prod.temp.yml
        exit 1
    fi
fi
...
if [ ! \( $BUILD_EXIT_CODE -ne 0 -a -n "$CI" \) ]; then
    ...
    if [ -n "$CI" ]; then
        ...
        BACKEND_RUNNING=$($DOCKER_COMPOSE_CMD -f ../config/podman-compose.prod.temp.yml ps | grep backend | grep -c "Up")
        FRONTEND_RUNNING=$($DOCKER_COMPOSE_CMD -f ../config/podman-compose.prod.temp.yml ps | grep frontend | grep -c "Up")
    fi
    ...
    if [ $BACKEND_RUNNING -eq 0 ] || [ $FRONTEND_RUNNING -eq 0 ]; then
        ...
        $DOCKER_COMPOSE_CMD -f ../config/podman-compose.prod.temp.yml logs
